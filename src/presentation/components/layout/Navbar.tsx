...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 glass-panel border-b border-[#D4A373]/20 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <a
            ...
            className="hidden lg:flex items-center gap-2 text-xs font-semibold text-stone-300 bg-stone-800/80 px-3 py-2 rounded-full border border-stone-700/50 hover:border-[#D4A373]/50 transition-all"
          >
          ...
          <button
            ...
            className="relative p-2.5 bg-gradient-to-r from-[#D4A373] to-[#BC8A5F] hover:from-[#E5B484] hover:to-[#CD9B70] text-stone-950 font-bold rounded-full shadow-lg transition-all transform hover:scale-105 active:scale-95 flex items-center gap-2 px-4"
          >
            <ShoppingBag className="w-5 h-5" />
            <span className="text-sm font-extrabold">{cart.getTotalAmount()} ₽</span>
            {totalItems > 0 && (
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-stone-900/95 border-b border-stone-800 px-4 pt-4 pb-6 space-y-3"
          >
            {navLinks.map((link) => (
              <Link
                key={link.href}
                ...
                className="block text-base font-medium text-stone-200 hover:text-[#D4A373] py-2 border-b border-stone-800"
              >
                {link.label}
              </Link>
            ))}
